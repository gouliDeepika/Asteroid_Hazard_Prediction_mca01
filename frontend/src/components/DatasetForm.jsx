...
import { fetchAsteroids, fetchAsteroidById, predictUnified } from "../services/api";
...
function DatasetForm() {
  ...
  useEffect(() => {
    fetchAsteroids()
      ...
      .catch(() => alert("Failed to load asteroid list"));
  }, []);

  // Autofill when asteroid selected
  const handleSelect = async (id) => {
    ...
    try {
      const res = await fetchAsteroidById(id);
      const a = res.data;

      setFormData({
        absolute_magnitude: a.absolute_magnitude,
        estimated_diameter_min: a.estimated_diameter_min,
        estimated_diameter_max: a.estimated_diameter_max,
        relative_velocity: a.relative_velocity,
        miss_distance: a.miss_distance,
      });
    } catch {
      alert("Failed to fetch asteroid details");
    }
  };
  ...
  return (
    <div>
      <h3 className="text-xl font-semibold text-cyan-400 mb-6">
        📊 Dataset Asteroid (Editable)
      </h3>

      {/* Dropdown */}
      <div className="mb-6">
        <label className="text-sm text-gray-400">Select Asteroid</label>
        <select
          value={selectedId}
